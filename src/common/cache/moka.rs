...
pub async fn get_client_idp_config(client_id: &str) -> Result<Option<ClientIdpConfig>> {
    Ok(match CLIENT_IDP_CONFIG_CACHE.get(client_id).await {
        Some(client) => Some(client),
        None => match client_rpc::fetch_client_idp_config(client_id, None).await? {
            Some(ref config) => {
                CLIENT_IDP_CONFIG_CACHE
                    .insert(client_id.to_string(), config.clone())
                    .await;
                Some(config.clone())
            }
